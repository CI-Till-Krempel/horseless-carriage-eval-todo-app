...
def test_add_and_toggle_task(client):
    ...
    res_task = client.post(f'/lists/{list_id}/tasks', data={'description': 'Finish report'}, follow_redirects=True)
    assert b'Finish report' in res_task.data

    data = load_data()
    task_id = data['lists'][0]['tasks'][0]['id']

    res_toggle = client.post(f'/lists/{list_id}/tasks/{task_id}/toggle', follow_redirects=True)
...
def test_delete_task(client):
    ...
    data = load_data()
    list_id = data['lists'][0]['id']
    client.post(f'/lists/{list_id}/tasks', data={'description': 'Sweep floor'}, follow_redirects=True)
    
    data = load_data()
    task_id = data['lists'][0]['tasks'][0]['id']

    res_del = client.post(f'/lists/{list_id}/tasks/{task_id}/delete', follow_redirects=True)
